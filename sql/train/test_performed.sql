SELECT lab_table.test_key, lab_table.result_key,
    lab_table.result_full_description, lab_table.test_performed,
    dbo_table.candidates
FROM (
    SELECT test_key, result_key, result_full_description,
        LOWER(test_performed) AS test_performed
    FROM lab.dim_test_result_output_v2 AS table_0
    WHERE test_performed IS NOT NULL
        AND test_performed <> 'pending'
